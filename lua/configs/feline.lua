...
function M.config()
  local present, feline = pcall(require, "feline") if present then
        ...
        active = {
          {
            ...
            { provider = "git_branch", hl = hl.fg("Conditional", { fg = C.purple_1, style = "bold" }), icon = " " , hl = hl.mode() },
            { provider = provider.spacer(), hl = hl.mode() },
            ...
            { provider = { 
              name = "file_info", 
              opts = { filetype_icon = false, type = "full-path" } },
